        ...
        <div v-for="(item, key) in this.data" :key="key" class="bg-gray-200 rounded-2xl h-72 flex relative overflow-hidden">
          ...
          <div class="flex relative text-end w-3/5 h-full">
            ...
            <div class="absolute w-full pr-3 lg:pr-6 py-8">
              ...
              <router-link :to="'/pengumuman/'+item.slug" class="px-4 bg-amber-400 text-black text-xs lg:text-base rounded-lg">Read More</router-link>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    getPengumuman: function () {
      axios.get(process.env.VUE_APP_BASE_URL + 'api/publikasi/front-pengumuman/')
          .then(resp => {
